...
r2stl_geo <- function(shapefile=NULL, variable=NULL, gridResolution = 100, keepXYratio = TRUE, zRatio = 0.5, filename='3d-R-object.stl', object.name='r2stl-object', min.height=0.008, show.persp=FALSE, strict.stl=FALSE, reliefLayer = NULL, interpolate = 0) {
  ...
  if(class(reliefLayer)=='SpatialPolygonsDataFrame'){
    ...
    #Need to normalise relief layer's values on the z axis
    #Ideally it'd be added after the normalisation below, but FAFF
    #So instead use the raster range to make it proportional
    
    # useRaster <- useRaster + reliefRaster
    
    # useRaster <- useRaster + (reliefLayer * 
    #                             max(values(useRaster)) - min(values(useRaster))
    # 
  }
  ...
  #Repeat for relief raster to apply after the shapefile's been normalised...
  zr <- t(as.matrix(reliefRaster))
  #Aaand flip
  zr <- zr[nrow(zr):1,]
  zr[is.na(zr)] <- 0
  ...
  #Back to orig with some edits for proportions
	...
	#Also return a copy of the final raster
	
	#use zz as this includes the reliefRaster.
	#values(useRaster) <- zz
